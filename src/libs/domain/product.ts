...
export class Product {
  ...
  updateVariants(
    ...
    product_variants.forEach((variant) => {
      ...
      const { property_list, metadata } = Product.createPropertyList(updated);
    ...
    const newVariantList = variantList
      .filter((variant) => !product_variants.find((v) => v.sku === variant.sku))
      .map((variant) => {
        const { property_list, metadata } = Product.createPropertyList(variant);
  ...
  serialize(): SerializedProduct {
    ...
    const rawVariants = product_variants.map((variant) => variant.serialize());
  ...
  private validateVariant() {
    ...
    for (const variant of this.props.product_variants) {
      ...
      if (variantSet.has(variant.variantValue) || skuCodeSet.has(variant.sku)) {
  ...
  static createProduct(dto: CreateProductDTO) {
    ...
    const status = isPublished ? ProductStatus.Published : ProductStatus.Draft;
